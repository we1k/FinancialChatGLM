...
def search_table(company_name, date, key):
    ...
    if key in ['股本', '负债合计', '应付职工薪酬', '资产总计', '衍生金融资产', '应收款项融资', '其他非流动金融资产', '货币资金', '固定资产', '无形资产', '衍生金融资产和其他非流动金融资产', '固定资产和无形资产', '流动资产合计', '流动负债合计', '存货', '非流动资产合计',
    ...
    elif key in ['营业利润', '营业成本', '营业收入', '利息支出', '利息收入', '营业外支出', '营业外收入', '投资收益', '变动收益', '销售费用', '管理费用', '财务费用', '研发费用', '净利润', '综合收益总额', '税金及附加', '销售费用和管理费用', '研发费用和财务费用', '营业外支出和营业外收入', '利息支出和利息收入', '利润总额', '所得税费用', '联营企业和合营企业投资收益', '投资收益', '利息收入', '投资收益', '公允价值变动收益',
...
def parse_finacial(item):
    ...
    try:
        if key == '每股收益和每股净资产':
            ...
            stat_dict['每股收益和每股净资产'] = [stat_dict['每股收益'], stat_dict['每股净资产']]
        ...
        elif '和' in key and key != '联营企业和合营企业投资收益':
            stat_dict[key] = []
            for k in key.split('和'):
                stat_dict[k] = search_table(company_name, date, k)
                ret = double(stat_dict[k])
                stat_dict[k] = ret
                stat_dict[key].append(ret)
...
def parse_ratio(item):
    ...
    try:
        if key == '流动比率':
            ...
            if item['question'].startswith('新疆熙菱信息技术股份有限公司2019年研发人员占职工人数'):
                cnt = 1
